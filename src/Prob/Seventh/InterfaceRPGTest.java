...
class Adventure implements SkillSet{
    ...
    // 생성자
    public Adventure(int atk, int def,
                     int mag, int mdef, int dex, int vit, int mental) {
        this.atk = atk;
        this.def = def;
        this.mag = mag;
        this.mdef = mdef;
...
class Warrior extends Adventure {
    ...
    public void skill1() {
        /*바뀐 스킬 이펙트 구현*/
        System.out.println(
                "Damage: " + getAtk() + 3.3 + getDex() * 0.5
        );
    }

    public void skill2() {
        System.out.println(
                "Stun 1.0s, Damage: " + getAtk() + 1.2 +
                        getDex() * 0.1
        );
    }
...
public class InterfaceRPGTest {
    public static void main(String[] args) {
        ...
        // 객체를 출력할 때 toString()이 있으면 원하는 형로 출력이 가능하다.
